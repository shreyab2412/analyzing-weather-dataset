...
def clear(df,col,val):
    ...
    """
    value_counts=weather[(weather[col]==val)].count()
    return value_counts
...
def instances_based_condition(df,col1,val1,col2,val2):
    ...
    """
    instance=weather[(weather[col1]>val1)&(weather[col2]==val2)]
    return instance
...
def agg_values_ina_month(df,date_col,agg_col,agg):
    ...
    #agg={agg_col:mean,agg_col:max,agg_col:min,agg_col:sum,agg_col:len}
    aggregated_value=pd.pivot_table(weather,index=date_col,values=agg_col,aggfunc=agg)
...
def group_values(df,col1,agg1):
   ...
   Keyword arguments:
    ...
    grouping=weather.groupby(col1).agg(agg1[col1])
    return grouping
...
weat=pd.read_csv(path)
weather=pd.DataFrame(weat)
...
s=clear(weather,'Weather','Cloudy')
print(s)
...
agg={'Temp (C)':[np.mean,sum,min,max]}
print(agg_values_ina_month(weather,'Date/Time','Temp (C)',agg))
